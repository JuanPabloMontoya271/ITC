...
Element DateProcessor::content()
{
    ...
    int key_ip_ = StringManipulation::toInt(StringManipulation::split(ip_, '.')[0]);
    int numeric_ip = StringManipulation::parse_ip(password);
    std::cout << "key_ip : " << key_ip_<< std::endl;
    Element element{ d, password, message, numeric_ip, key_ip_, ip_};
    return element;
